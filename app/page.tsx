...
const fallTimeline = [
  ...
  { date: "Fall 2025", title: "Biola Housing RA Hiring System", desc: "AI Lab Associate Megan Lai built a custom hiring system for Biola Housing to streamline their Resident Assistant selection. A custom Apps Script form captures asynchronous candidate interviews, then synthesizes those responses with notes from two in-person interviewers into a single spreadsheet — letting reviewers compare candidates at a glance while preserving the nuance of each one's strengths and weaknesses. The system automated 30–50 hours of manual work previously spent searching through handwritten notes for over 100 candidates.", img: "/images/Biola Housing.png" },
...
type TimelineItem = {
  ...
  desc: string;
  img: string | string[];
  imgPosition?: string | (string | undefined)[];
    ...
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl overflow-hidden flex flex-col group-hover:border-[var(--gold)]/40 transition-colors duration-300">
      <div className="h-44 flex-shrink-0 overflow-hidden relative">
        <img
          ...
          style={{ objectPosition: (Array.isArray(item.imgPosition) ? item.imgPosition[imgIdx] : item.imgPosition) ?? "center" }}
        />
        {images.length > 1 && (
          <>
            <button
